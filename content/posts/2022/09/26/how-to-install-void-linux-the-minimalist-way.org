...
The absolute minimum you need is a boot partition and root partition (for UEFI/GPT). You can further customize to have /home or /usr as seperate partition or even a seperate drive. For example, boot and root on SSD and home on HDD.
...
/dev/sdb1 encrypted using cryptsetup but with keyfiles
...
Use wipefs to wipe filesystem to change from GPT to MBR or vice versa.
...
mkfs allows you to actually format the parition to filesystem that you want.
...
Create keyfile using dd command.
...
Use dd to create keyfile, create bootable USB and many others.

Use this to create keyfile
...
If you are using keyfiles then copy the keyfile to somewhre in /mnt.
...
If you are using keyfiles then also add an entry to crypttab to auto decrypt your partition using keyfile. home is name of the partition, the part after UUID= is a function to get UUID of the partition and the last part is the path to keyfile.
...
Mount some devices and stuff thats required.
...
** Setup locale, timezone, network services etc
:PROPERTIES:
...
Note: fstab is pronounced fs tab (for filesystem table). Not f stab :).
...
If using keyfile for home or other partition then make sure to use the one that is going to be mounted. For example, /dev/mapper/home if not using LVM and /dev/mapper/pool-home if using LVM.
...
Most bios are buggy and they assume the directory and filenames to boot from. To solve this, copy the necessary stuff to the directory that windows use. And rename them to the one that windows use.
...
Dont install tor browser if on musl. Use flatpak instead.
...
* Want to help?
:PROPERTIES:
:ID:       95113f7a-531f-4049-b8db-1949fa830698
:PUBDATE:  <2022-08-09 Tue 18:33>
:END:
